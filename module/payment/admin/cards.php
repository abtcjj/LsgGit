		...
		if($_POST["act"]=='save')
		{
			...
			if($total_price>0)
			{
				...
				for($j=0;$j<$num;$j++)
				{
					...
					,`statu` ,`creat_time` ,`pic`,`stime`,`etime`,give_price )VALUES('$card_num','$total_price','$password', '0', '".time()."', '$_POST[pic]','".($_POST['stime']?strtotime($_POST['stime']):0)."','".($_POST['etime']?strtotime($_POST['etime']):0)."','$_POST[give_price]')";
					$db->query($sql);
				}	

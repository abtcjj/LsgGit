<?php
	...
	if(!empty($_POST['mid'])&&is_numeric($_POST['mid'])&&!empty($_POST['uname'])&&$_POST['op']=="add")
	{
		...
		if($db->num_rows()<=0)
		{			
			$db->query("update ".MEMBERCOUNT." set friend=friend+1 where member_id='".$re['userid']."'");	
			$db->query("update ".MEMBERCOUNT." set fan=fan+1 where member_id='".$_POST['mid']."'");	
	...
	if(!empty($_POST['fid'])&&is_numeric($_POST['fid'])&&$_POST['op']=="del")
	{
		...
		$db->query("update ".MEMBERCOUNT." set friend=friend-1 where member_id='".$re['uid']."'");	
		$db->query("update ".MEMBERCOUNT." set fan=fan-1 where member_id='".$re['fuid']."'");

...
class shop
{
	...
	function EditShop($id)
	{
		...
		if(!empty($earnest))
		{
			...
			$post['buyer_email']=$id;//
			$post['seller_email']='0';//
			$post['order_id']='E'.time();//外部订单号
			$post['price']=$earnest*(-1);//订单总价，单价元
			$post['mold']='6';
			$post['earnest']=$flag;
			$post['s']=$_POST['desc'];
	...
	function get_shop_info($id)
	{
		...
		$sql="select b.*,a.regtime,a.lastLoginTime,a.logo as plogo,a.ip,sellerpoints,buyerpoints,a.name,a.userid,b.userid as uid

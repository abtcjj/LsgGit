...
class AddField
{
					...
					if($ob['type']==2)
					{
						if(in_array($itemvalue[0],$nv))
						{
							...
							if($p[($ob['j']-1)]==$itemvalue[0])
							{
								...
								$str1 = "class='checked' genre=\"{'num':'$ob[j]','id':'$itemvalue[0]'}\" ";
							}
							$selectnum.='<li title='.$itemvalue[1].'><a '.$cimg.' onclick="selectSpec(\''.$ob['j'].'\',this,\''.$itemvalue[0].'\');"  '.$str1.'>'.$itemvalue[1].'<i></i></a></li>';
						}
					}
					else
					{
	...
	function addtabletr($ob)
	{
		$type = $ob["displayType"];
		$htmlstr="";
		$returnstr ="<tr><td>".$ob["fieldDisplay"]."</td><td>";
		$value=$ob["defaultValue"];
	...
	function addtabletr1($ob)
	{
		...
		for($ii=0;$ii<count($numarray);$ii++)
		{
			...
			$selectnum.="<li><span genre='chk'><input class='checkbox' $str type=\"checkbox\" name=\"".$ob["fieldName"]."[]\" value=\"".$itemvalue[0]."\" id=\"".$ob["fieldName"].$ii."\" genre=\"".$itemvalue[1]."\" ></span><span><label for=\"".$ob["fieldName"].$ii."\">".$itemvalue[1]."</label></span></li>";
	...
	function echoforeach($len,$sign)
	{
		if($len < $sign){
			...
			$abc.= "str +='<td><input class=\"text\" type=\"text\" name=\"spec['+spec_bunch+'][price]\" data_type=\"price\" genre=\"'+spec_bunch+'|price\" value=\"\" /></td><td><input class=\"text\" type=\"text\" name=\"spec['+spec_bunch+'][stock]\" data_type=\"stock\" genre=\"'+spec_bunch+'|stock\" value=\"\" /></td><td><input class=\"text\" type=\"text\" name=\"spec['+spec_bunch+'][sku]\" data_type=\"sku\" genre=\"'+spec_bunch+'|sku\" value=\"\" /></td></tr>';";
			for($i=0; $i< $len; $i++){
				$abc.= "}";
			}
		}

...
class pro
{
	...
	function get_brand($id,$sbrand=NULL)
	{
		...
		if(!empty($brand))
		{
			...
			{
				$s='selected="selected"';
				$input='&nbsp;<input maxlength="20" type="text" name="brand" value="'.$sbrand.'" />';
			}
			$str="<option $s value='other'>其他</option>";
			return '<select select="select" name="brand" id="brand" />'.$op.$str.'</select>'.$input;
		}
		else
	...
	{	
		...
		$rebate=$_POST['rebate']?$_POST['rebate']:"0.00";

		$promotion_tips=htmlspecialchars($_POST["promotion_tips"]);
		$str=substr($_POST['match'],'0','-1');
		$_POST['match']=empty($_POST['match'])?NULL:$str;
		...
		$sql="INSERT INTO ".PRO." (userid,user,catid,pname,brand,market_price,price,uptime,pic,pic_more,stime_type,stime,validTime,province,city,amount,code,ptype,keywords,credit,custom_cat_id,point,freight_type,freight,post_price,express_price,ems_price,area,areaid,promotion_tips,`match`,statu,rebate) 
		VALUES
	  ...
	  '$_POST[validTime]','$_POST[province]','$_POST[city]','$_POST[amount]','$_POST[code]',
		'$_POST[ptype]','$_POST[keywords]','$credit','$_POST[custom_cat]','$_POST[point]','$_POST[freight_type]','$_POST[freight]','$_POST[post_price]','$_POST[express_price]','$_POST[ems_price]','$_POST[t]','$_POST[area]','$promotion_tips','$_POST[match]','0','$rebate')";
		$re=$this->db->query($sql);
		$id=$this->db->lastid();
	...
	function edit_pro()
	{
		...
		if(!empty($_POST['spec']))
		{
			...
			foreach($_POST['spec'] as $key=>$v)
			{
				...
				if(!empty($v))
				{
					$sql="insert into ".SETMEAL." (pid,setmeal,price,stock,sku,property_value_id) 
					values ('$id','".$name."','".$v['price']."','".$v['stock']."','".$v['sku']."','".$pids."')";
					$this->db->query($sql);
				}
			}	
		}
	...
	function pro_detail($id)
	{
		...
		{	
			...
			$re['match_id'] = $re['match']?explode(',',$re['match']):'';
			if($re['match_id'])
			{
				foreach($re['match_id'] as $key=>$val)
				{
					if($val)
					{
						$sql="SELECT id,pic,pname,price,market_price,amount FROM ".PRO." where id='$val' order by id desc";
						$this->db->query($sql);

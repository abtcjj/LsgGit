...
class order
{
	...
	function buyorder($status='')
	{
		...
		if(is_numeric($status))
		    $sql="select a.*,b.company from ".ORDER." a left join ".SHOP." b on a.seller_id=b.userid 
				where a.userid='".$buid."' and seller_id!='' and a.status=".$status." order by a.id desc";
		else
            $sql="select a.*,b.company,b.user from ".ORDER." a left join ".SHOP." b on a.seller_id=b.userid 
	...
	function sellorder($status='')
	{
		global $buid;
		if(is_numeric($status))
		    $sql="select a.*,b.company,b.user from ".ORDER." a left join ".SHOP." b on a.buyer_id=b.userid 
	...
	{
		global $buid;
		$sql="select a.*,b.name as shipping_name,b.area as shipping_area,b.addr as shipping_address,b.mobile as shipping_mobile ,b.tel as shipping_tel from ".ORDER." a left join ".SHIPPINGADDR." b on a.deliver_addr_id=b.id where order_id='$id' and a.userid='$buid'";
		$this->db->query($sql);
        $re=$this->db->fetchRow();
	...
	function get_next_action($type,$statu,$oid,$admin='0')
	{
		...
		if($statu<5&&$statu>0)
		{
			...
			if(isset($index))
			{
				...
				if($index==1)
				{
					$str="<a target='_blank' class='buttons' href='pay/pay.php?tradeNo=$oid'>$order_action[$index]</a>&nbsp;";
				...
				if($index==3)
				{
					$str="<a class='buttons' href='main.php?m=product&s=admin_receipt&id=$oid'>$order_action[$index]</a>";	
				}
				return $str;
	...
	function add_commission($order_id,$seller)
	{
		...
		foreach($re as $k=>$v)
		{
			if($v['rebate']>0)
			{
				...
				$post['mold']='5';
				$post['s1']='订单'.$order_id.'商品'.$v['name'].'返利收入';
				$post['s']='订单'.$order_id.'商品'.$v['name'].'返利支出';
		...
		if($one_price>0)
		{
			...
			$post['s1']='订单'.$order_id.'拥金收入';
			$post['s']='订单'.$order_id.'拥金支出';
			pay_get_url($post,true);//跳转至订单生成页面
			//--------------
		}

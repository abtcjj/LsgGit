<?php
	...
	if($_GET['operation']=="add" or $_GET['operation']=="edit")
	{
		...
		{	
			...
			$_POST['catid']=$_POST['catid']?$_POST['catid']:"0";
			$_POST['market_price']=$_POST['market_price']?$_POST['market_price']:"0";
			$_POST['price']=$_POST['price']?$_POST['price']:"0";
			$_POST['t']=$_POST['t']?$_POST['t']:"";
			$_POST['is_virtual']=$_POST['is_virtual']?$_POST['is_virtual']:"false";
			$_POST['member_id']=$_POST['member_id']?$_POST['member_id']:"0";
			//添加
			if($_POST["act"]=='save')
			{
				$sql="insert into ".TG." (`catid`,`name`,`content`,`pic`,`market_price`,`price`,`hits`,`sell_amount`,`limit_quantity`,`virtual_quantity`,`status`,`create_time`,`stock`,`provinceid`,`cityid`,`areaid`,`area`,`displayorder`,`is_virtual`,`member_id`) values 
			('$_POST[catid]','$_POST[name]','$_POST[content]','$_POST[pic]','$_POST[market_price]','$_POST[price]','$_POST[hits]','$_POST[sell_amount]','$_POST[limit_quantity]','$_POST[virtual_quantity]','$_POST[status]','$time','$_POST[stock]','$_POST[province]','$_POST[city]','$_POST[area]','$_POST[t]','255','$_POST[is_virtual]','$_POST[member_id]')";
				$db->query($sql);
			}
			//修改
			if($_POST["act"]=='edit' and is_numeric($_POST['id']))
			{
				$sql="update ".TG." set name='$_POST[name]',catid='$_POST[catid]',content='$_POST[content]',pic='$_POST[pic]',market_price='$_POST[market_price]',price='$_POST[price]',sell_amount='$_POST[sell_amount]',limit_quantity='$_POST[limit_quantity]',virtual_quantity='$_POST[virtual_quantity]',stock='$_POST[stock]',status='$_POST[status]',hits='$_POST[hits]',provinceid='$_POST[province]',cityid='$_POST[city]',areaid='$_POST[area]',area='$_POST[t]',is_virtual='$_POST[is_virtual]',member_id='$_POST[member_id]' where id='".$_POST['id']."'";
				$db->query($sql);
				unset($_GET['editid']);
			}
		...
		//获取提供厂商
		$sql="select id,name from ".TGMEMBER;
		$db->query($sql);
		$member=$db->getRows();

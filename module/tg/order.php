...
if($buid&&$_GET['oid']*1)
{
	...
	msg("main.php?m=tg&s=admin_tg_order&cg_u_type=1");
...
if($buid&&$_GET['id']*1>0)
{
	...
	if(!empty($_POST))
	{
		...
		if($re)
		{
			...
			$post['return_url']='index.php?m=tg&s=order&oid='.$order_id.'&id='.$_GET['id'];//返回地址
			$post['notify_url']='index.php?m=tg&s=order&oid='.$order_id.'&id='.$_GET['id'];//异步返回地址。
			$post['name']="团购订单:".$order_id."消费";//备注
			pay_get_url($post,true);//跳转至订单生成页面
			msg($config['weburl']."/main.php?m=tg&s=admin_tg_order");//订单提交成功
			//----------------
		}
	}
}

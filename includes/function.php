...
function pay_get_url($post,$return=NULL)
{
	...
	if(empty($return))
		header("Location: $config[weburl]/pay/api/gateway.php?".$str);
	else
	{
		$url="$config[weburl]/pay/api/gateway.php?".$str;
		$re=get_url_content($url);
		return $re;
	}
}
...
function sms_send($id,$arr=array())
{
	$sql="select * from ".REMIND." where id = 'id' order by id ";
	$db->query($sql);
	$de=$db->getRows();

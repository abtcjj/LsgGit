...
class shop{
	...
	function site_spread($uid='')
	{
		...
		else
		{
			...
			$this->db->query($sql);
			$sql="update ".MEMBER." set point=point+1 where userid='$uid'";
			$this->db->query($sql);
		}
		return true;
	}
	...
	{	
		...
		if(!is_numeric($uid))
			$sql="select userid,statu,name,qq,email,mobile,area,skype,logo as plogo,regtime,sellerpoints,buyerpoints from ".MEMBER." WHERE user='$uid'";
		else
			$sql="select userid,statu,name,qq,email,mobile,area,skype,logo as plogo,regtime,sellerpoints,buyerpoints from ".MEMBER." WHERE userid='$uid'";
		$this ->db ->query($sql);

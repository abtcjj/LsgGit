...
{	
	...
	else
	{
		...
		$shopconfig["hometitle"].=($shopconfig["hometitle"]?'-':"").$company['company'];
		...
		else
		{
			...
			$sql="select * from ".PRO." where shop_rec=1 and userid='$_GET[uid]' order by id limit 0,6";
			$db->query($sql);
			$re=$db->getRows();
